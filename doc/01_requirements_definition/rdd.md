...
### 2. 🌙 表示ページ（`/view/[id]`）
- **機能概要**: 生成された詩とアニメーション/画像を表示する専用ページ
- **主な要素**:
  - 詩の美しい表示
  - DALL-E生成背景画像の表示
  - p5.jsによるオーバーレイアニメーション（オプション）
  - SNS共有ボタン
  - OGP対応metaタグ（生成画像を含む）
...
#### フォールバック戦略
1. **DALL-E失敗時**: p5.jsアニメーション背景にフォールバック
